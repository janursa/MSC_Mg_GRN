...
from proteomics_MSC.imports import MODELSELECTION_DIR, MODELSELECTION_BASELINE_DIR
from proteomics_MSC.common_tools import serif_font, flatten
from proteomics_MSC.common_tools.links import choose_top_quantile, normalize_links
...
def get_baseline_scores(data_type: str) -> Tuple[List[float], List[List[float]]]:
    ...
    ep_scores = np.loadtxt(Path(MODELSELECTION_BASELINE_DIR) / f'ep_scores_{data_type}.csv', delimiter=',')
    ep_scores_series = np.loadtxt(Path(MODELSELECTION_BASELINE_DIR) / f'ep_scores_series_{data_type}.csv', delimiter=',')
...
def save_baseline_scores(ep_scores: List[float], ep_scores_series: List[List[float]], DE_type: str) -> None:
    if not os.path.isdir(MODELSELECTION_BASELINE_DIR):
        os.makedirs(MODELSELECTION_BASELINE_DIR)
    to_save = f'{Path(MODELSELECTION_BASELINE_DIR)}/ep_scores_{DE_type}.csv'
    np.savetxt(to_save, np.asarray(ep_scores),
               delimiter=',')
    print(f'output -> {to_save}')
    to_save = f'{Path(MODELSELECTION_BASELINE_DIR)}/ep_scores_series_{DE_type}.csv'
    np.savetxt(to_save, np.asarray(ep_scores_series),
               delimiter=',')
    print(f'output -> {to_save}')
...
class ViolinPlot:
    ...
    def plot(cls,
        ...
        _, medians, _ = np.percentile(np.asarray(data_stack), [25, 50, 75], axis=1)
...
def string_functions_analysis(my_genes, string_api="https://version-11-5.string-db.org/api"):
    """Obtain functional relationships"""
    output_format = "tsv"
    method = "network"
