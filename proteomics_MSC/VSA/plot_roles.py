...
from proteomics_MSC.common_tools.role_analysis import RolePlot
...
def plot_roles(studies_names, data_ctr, data_treatment, top_role_changes, gene_names):
    ...
    # - top role change
    RolePlot.plot_role_change(top_role_changes, ax=axes[2], custom_annotation=custom_annotation_role_change,
...
if __name__ == '__main__':
    ...
    for model_name in F_selected_models():
        ...
        top_role_change = pd.read_csv(Path(VSA_VERBOSE_DIR) / f'top_role_change_{model_name}.csv', index_col=False)
        # - robust role change genes
        with open(f"{VSA_DIR}/robust_genes_{model_name}.csv", 'r') as file:
            robust_genes = list(np.loadtxt(file, dtype=str, delimiter=','))
        robust_genes_df = top_role_change.loc[top_role_change['Entry'].isin(robust_genes), :].reset_index()
        # - plot roles for ctr and sample, top role changes and critical role changes in one plot
        fig = plot_roles(studies, vsa_results_studies_ctr, vsa_results_studies_sample, robust_genes_df,
                         genenames)
        to_save = os.path.join(VSA_PLOT_DIR, f'role_changes_{model_name}.png')
