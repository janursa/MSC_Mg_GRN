...
class Links:
    def batch_GRN(study, method, data, param, gene_names, test_size, time_points, param_unique, istart=None, iend=None,OUTPUT_DIR=''):
        ...
        for i in range(istart, iend):
            ...
            train_scores, oob_scores, links_df = Links.network_inference(data, gene_names, time_points, test_size, param, param_unique)
            Links.read_write_links(links=links_df, study=study, mode='write_links', method=method, OUTPUT_DIR=OUTPUT_DIR, i=i)
            Links.read_write_links(oob_scores=oob_scores, study=study, mode='write_oob_scores', method=method, OUTPUT_DIR=OUTPUT_DIR, i=i)
            Links.read_write_links(train_scores=train_scores, study=study, mode='write_train_scores', method=method, OUTPUT_DIR=OUTPUT_DIR, i=i)
    ...
    def pool_links(study, protnames, output_dir, n, method='') -> pd.DataFrame:
        ...
        links_list = [Links.read_write_links(study=study, mode='read_links', i=i, OUTPUT_DIR=output_dir, method=method) for i in range(n)]
...
class Calibration:
    ...
    def batch_tune(study, method, data, param, gene_names, test_size, time_points, param_grid, specs, istart=None, iend=None,OUTPUT_DIR=''):
        ...
        for i in range(istart, iend):
            ...
            best_scores, best_params = calibration.search(data, gene_names, time_points, test_size, param, param_grid, specs)
            print(f'mean best score {np.mean(best_scores)}')
            calibration.read_write_oo(study, method=method, mode='write', bestparams=best_params, bestscores=best_scores,
    ...
    def pool_oo(study, method, n=100, OUTPUT_DIR=''):
        ...
        best_scoress = []
        for i in range(n):
            best_params, best_scores = calibration.read_write_oo(study, method=method, mode='read', OUTPUT_DIR=OUTPUT_DIR, i=i)
    ...
    def plot_oo(method, priors, protnames, OUTPUT_DIR):
        ...
        bestparams_ctr, bestscores_ctr, bestparams_pool_ctr, bestscores_pool_ctr = calibration.retreive_data(
            'ctr', method=method, read_dir=dir)
        bestparams_mg, bestscores_mg, bestparams_pool_mg, bestscores_pool_mg = calibration.retreive_data('mg',
                                                                                                               method=method,
                                                                                                               read_dir=dir)

        # - best score pool
        fig = calibration.plot_oobscores_pool(bestscores_pool_ctr, bestscores_pool_mg, protnames)
        fig.savefig(os.path.join(dir, 'bestscores_pool.png'), dpi=300, transparent=True,
                    facecolor='white')
        # - best param pool
        fig = calibration.plot_bestparams_pool(bestparams_pool_ctr, priors, protnames)
        fig.savefig(os.path.join(dir, 'bestparams_pool_ctr.png'), dpi=300, transparent=True, facecolor='white')

        fig = calibration.plot_bestparams_pool(bestparams_pool_mg, priors, protnames)
        fig.savefig(os.path.join(dir, 'bestparams_pool_mg.png'), dpi=300, transparent=True, facecolor='white')

        # - best score mean
        fig = calibration.plot_oobscores(bestscores_ctr, bestscores_mg)
        fig.savefig(os.path.join(dir, 'bestscores.png'), dpi=300, transparent=True, facecolor='white')
        # - best param mean
        fig = calibration.plot_bestparams(bestparams_ctr, bestparams_mg, priors=priors)
