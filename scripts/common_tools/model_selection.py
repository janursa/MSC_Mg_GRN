...
from imports import MODELSELECTION_DIR, MODELSELECTION_BASELINE_DIR
from common_tools import serif_font, flatten
from common_tools.links import choose_top_quantile, normalize_links
...
def save_baseline_scores(ep_scores: List[float], ep_scores_series: List[List[float]], DE_type: str) -> None:
    ...
    np.savetxt(f'{Path(MODELSELECTION_BASELINE_DIR)}/ep_scores_{DE_type}.csv', np.asarray(ep_scores),
               delimiter=',')
    np.savetxt(f'{Path(MODELSELECTION_BASELINE_DIR)}/ep_scores_series_{DE_type}.csv', np.asarray(ep_scores_series),
               delimiter=',')
