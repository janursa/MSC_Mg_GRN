...
from imports import  F_selected_models, RANDOM_FEATURE_DIR
from model_selection.md_aux import determine_sig_flag, retreieve_scores, is_single_value, ViolinPlot
...
def calculate_scores_vs_random(selected_models):
    ...
    for selected_model in selected_models:
        ...
        with open(Path(RANDOM_FEATURE_DIR) / f'ep_scores_random_{selected_model}.csv', 'r') as ff:
...
if __name__ == '__main__':
    ...
    scores_dist = [np.repeat(score, n_repeat) if is_single_value(score) else score for score in ep_scores]
    ...
    fig.savefig(os.path.join(RANDOM_FEATURE_DIR, f'violinplot.png'), dpi=300, transparent=True)
    fig.savefig(os.path.join(RANDOM_FEATURE_DIR, f'violinplot.pdf'))
