...
class dynGENIE3:
    def __self__(TS_data,time_points,alpha='from_data',SS_data=None,gene_names=None,regulators='all',tree_method='RF',K='sqrt',ntrees=1000,compute_quality_scores=False,save_models=False,nthreads=1,max_depth=None):
    ...
    if nthreads > 1:
        ...
        input_data = [[TS_data,time_points,SS_data,i,alphas[i],input_idx,tree_method,K,ntrees,compute_quality_scores,save_models,max_depth] for i in range(ngenes)]
        ...
        for out in alloutput:
            ...
            (vi,prediction_score_i,stability_score_i,treeEstimator,score_train) = out[1]
            VIM[i,:] = vi
        ...
        for i in range(ngenes):
            ...
            (vi,prediction_score_i,stability_score_i,treeEstimator,score_train) = dynGENIE3_single(TS_data,time_points,SS_data,i,alphas[i],input_idx,tree_method,K,ntrees,compute_quality_scores,save_models,max_depth)
            VIM[i,:] = vi
            scores_train[i] = score_train
            if compute_quality_scores:
                if tree_method == 'RF':
    ...
    return VIM, alphas, prediction_score, stability_score, treeEstimators,scores_train



def wr_dynGENIE3_single(args):
    return([args[3], dynGENIE3_single(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], args[9], args[10],args[11])])
...
def dynGENIE3_single(TS_data,time_points,SS_data,output_idx,alpha,input_idx,tree_method,K,ntrees,compute_quality_scores,save_models,max_depth):
    ...
    if tree_method == 'RF':
        treeEstimator = RandomForestRegressor(n_estimators=ntrees,max_features=max_features,oob_score=oob_score,max_depth=max_depth)
    elif tree_method == 'ET':
    ...
    score_train = treeEstimator.score(input_all,output_all)
    ...
    if save_models: 
        ...
        return vi, prediction_score_oob, stability_score, [],score_train
