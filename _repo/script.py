...
df_dir = os.path.join(main_dir,'data','omics_1.csv')
if 'original_df' not in locals():
    print("data not found")
    original_df = pd.read_excel(original_df_dir)

## tailor the data  
# dates = [1,2,3,4,7,8,10,11] # in dayx
dates = [1] # in dayx
# dates = [1] # in dayx
...
# db['Sites'] = ""
# db['Effect'] = ""
for ii in dates: #rename the data for control
    tag = prefix + str(ii) + '_0'
    df['ctr_{}'.format(ii)] = original_df[tag]
# print(db.loc[[40]])
for ii in dates: #rename the data for Mg
